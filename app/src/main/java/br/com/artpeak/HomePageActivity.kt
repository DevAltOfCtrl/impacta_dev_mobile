...
class HomePageActivity : NavigationDrawerActivity() {
    ...
    fun onClickTatuagem(tatuagem: Tatuagem) {
        ...
        intent.putExtra("tatuagem", tatuagem)
    ...
    fun taskTatuagem() {
        Thread {
            this.tatuagens = TatuagemService.get_tatu()
            runOnUiThread {
                recyclerTatuagens?.adapter = TatuagemAdapter(tatuagens) { onClickTatuagem(it) }
                enviaNotificacao(this.tatuagens.get(0))
            }
        }.start()
    }

    fun enviaNotificacao(tatuagem: Tatuagem) {
        val intent = Intent(this, TatuagenActivity::class.java)
        intent.putExtra("tatuagem", tatuagem)
        NotificationUtil.create(1, intent, "ArtPeak", "Você tem uma nona foto  ${tatuagem.nome}")
    }
