...
object NotificationUtil {
    ...
    fun createChannel() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            val contexto = ARTPeakApplication.getInstance().applicationContext
            val manager = contexto.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

            val c = NotificationChannel(CHANNEL_ID, "ArtPeak", NotificationManager.IMPORTANCE_HIGH)
    ...
    fun create (id: Int, intent: Intent, titulo: String, texto: String) {
        ...
        with(NotificationManagerCompat.from(ARTPeakApplication.getInstance())) {
            val n = builder.build()
            notify(id, n)
        }
